...
layout = html.Div([
    ...
    html.Div([
        dcc.Graph(id='myHist', figure={}, style={'width': '50%', 'display': 'inline-block'}),
        dcc.Graph(id='myScat', figure={}, style={'width': '50%', 'display': 'inline-block'}),
        html.Div([
...
def interactive_graph(value_method):
    ...
    fig_scat1 = px.scatter(dff, x='timestamp', y='response_time_ms', color='http_method', hover_data=['status_code', 'ip_address'])
    fig_urlscat = px.scatter(dff, x='url_accessed', y='response_time_ms', color='http_method', title='URL Response Time by Method',
                             hover_data=['status_code', 'ip_address'])
